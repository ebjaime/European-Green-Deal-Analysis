...
A linear model will result in exactly the same output since we are using a single regressor:
...
In the robustbase documentation we can find a note : "We strongly recommend using lmrob() instead of ltsReg [...]"
```{r}
...
# matlines(newdata$value_re, se.bands.lms,col="green",lwd=1,lty=2)

lines(newdata.rob$value_re, ghg.reg.lts$fit,col="lightblue",lwd=2)
matlines(newdata.rob$value_re, se.bands.lts, col="lightblue",lwd=1,lty=2)

```

The LTS model seems to be very similar by looking at the graph above. And their Sum of Square residuals is also very close, while the LMS estimator yields a worse fit in our model
...
lines(seq(1991,2019),ghg.l$fitted.values,lwd=2,col="green")
# New predictions LTS
points(seq(2020,2030),ghg.lts.preds$fit ,cex=0.7,pch=19, col ="lightblue")
lines(seq(1991,2019),fit_lts$fitted.values,lwd=2,col="lightblue")
...
c(Percentage_emissions.lts, Percentage_emissions)
