...
Let's try to quit gdp as well (residuals are gaussian, but simple doesn't work)
```{r}
...
colnames(vec_ghg.diff) <- c("value_ghg", "value_re","value_gdp","value_pop")
...
```{r}
Acf(ghg.gam$residuals,main="Complete model Acf")

```

```{r}
Pacf(ghg.gam$residuals,main="Complete model Pacf")
```
...
```{r}
Acf(ghg.gam.b$residuals,main="Baseline model Acf")
```

```{r}
Pacf(ghg.gam.b$residuals,main="Baseline model Pacf")
```
...
```{r}
#with baseline model
with(ghg, plot(seq(1990,2019) ,ValueGhg ,xlim=c(1990,2030),ylim=c(40,100),cex =.5,xlab="Year",ylab="Percentage" , col =" darkgrey ",main='Baseline Prediction - Ghg Emissions'))
...
```{r}
plot(seq(1991,2019), vec_ghg.diff$value_ghg, xlim=range(c(1990,2030)),cex =.5,xlab="Year",ylab="Diff percentage",col =" darkgrey ",main='Baseline conformal prediction - Ghg percentage variations',ylim=c(-10,10))
