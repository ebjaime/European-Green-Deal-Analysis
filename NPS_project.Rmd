...
```{r}
plot(seq(2005,2019,0.5), use_wind_elec.diff.total.s,cex =.5,xlab="Year",ylab="Terajoule",col =" darkgrey ",main='Wind Diff Electricity Use')
...
```{r}
plot(seq(2005,2019,0.5), use_hydro_elec.diff.total.s,cex =.5,xlab="Year",ylab="Terajoule",col =" darkgrey ",main='Hydro Diff Electricity Use')
...
```{r}
plot(seq(2005,2019,0.5), use_solar_elec.diff.total.s,cex =.5,xlab="Year",ylab="Terajoule",col =" darkgrey ",main='Solar Diff Electricity Use')
...
ghg.reg=predict(ghg.gam,newdata=newdata,se=T)
...
```{r}
#predictions
ghg.gam.preds <- predict(ghg.gam, newdata=vec_ghg_pred.diff, se=T)

with(vec_ghg.diff, plot(seq(1991,2019) ,value_ghg ,xlim=c(1990,2030),cex =.5,xlab="Year",ylab="Terajoule" , col =" darkgrey ",main='Prediction - Ghg Emissions'))
...
deltas=as.numeric(ghg.gam.preds$fit)
...
```{r}
plot(seq(1991,2019), vec_ghg.diff$value_ghg, xlim=range(c(1990,2030)),cex =.5,xlab="Year",ylab="Diff percentage",col =" darkgrey ",main='Conformal prediction - Ghg percentage variations',ylim=c(-10,10))
...
lines(seq(1991,2019),c_preds.ghg$fit, lwd =1, col =" green",lty =1)
```
...
## b. Test for YEAR outliers in predictions
...
nr.past.preds <- data.frame(
    scale(c(consump.oil.petr.gam$fitted.values,consump.oil.petr.gam.preds$fit)), 
    scale(c(consump.fossils.gam$fitted.values,consump.fossils.gam.preds$fit)),
    scale(c(consump.gas.gam$fitted.values,consump.gas.gam.preds$fit)))
...
```{r}
plot(seq(1991,2030), nr.past.preds[,1], col=ifelse(1:N%in%mcd_best.4,"black","red"),pch=19)
plot(seq(1991,2030), nr.past.preds[,2], col=ifelse(1:N%in%mcd_best.4,"black","red"),pch=19)
plot(seq(1991,2030), nr.past.preds[,3], col=ifelse(1:N%in%mcd_best.4,"black","red"),pch=19)
```


### b2. Renewable Energy consumption

REVIEW: "...However, as an estimator for univariate location and scale the MCD is
...
mcd_outl.5 <- which(fit_MCD.5$mcd.wt==0)

# Which are the outliers?
rownames(r.past.preds)[(1:N%in%mcd_outl.5)]
```
...
ghg.past.preds <- data.frame(
    c(c_preds$fit, c_preds$pred))

rownames(ghg.past.preds) <- c(seq(1991,2019,by=1/3), seq(2020,2030,by=1/3))
...
## p -> 1 (Renewable energy consumption)
alpha <- .75
fit_MCD.6 <- covMcd(x = r.past.preds, alpha = alpha, nsamp = "best")
plot(fit_MCD.6)

mcd_outl.6 <- which(fit_MCD.6$mcd.wt==0)

# Which are the outliers?
rownames(r.past.preds)[(1:N%in%mcd_outl.6)]
```
...
```{r}
reghg.past.preds <- data.frame(
     scale(c(c_preds$fit, c_preds$pred)),
     scale(c(consump.renew.gam$fitted.values, consump.renew.gam.preds$fit)), 
)
