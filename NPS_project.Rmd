...
REVIEW: With a single NR source, all are selected as ourliers
...
mcd_best.5 <- fit_MCD.5$best

N <- nrow(r.past.preds)
p <- ncol(r.past.preds)
plot(r.past.preds, col=ifelse(1:N%in%mcd_best.5,"black","red"),pch=19)

# Which are the outliers?
rownames(r.past.preds)[!(1:N%in%mcd_best.5)]
...
mcd_best.6 <- fit_MCD.6$best

N <- nrow(ghg.past.preds)
p <- ncol(ghg.past.preds)
plot(ghg.past.preds, col=ifelse(1:N%in%mcd_best.6,"black","red"),pch=19)

# Which are the outliers?
rownames(ghg.past.preds)[!(1:N%in%mcd_best.6)]
