...
colnames(energy.fossils.countries)=year.grid
colnames(energy.oil.petr.countries)=year.grid
colnames(energy.gas.countries)=year.grid
colnames(energy.renew.countries)=year.grid
```
...
So we can drop the interaction term
```{r}
...
Also here we can quit the interaction term
...
shapiro.test(model_gam$residuals)
```
Let's try quitting one covariate at a time. We start quitting the interaction term
```{r}
model_gam_reduced=gam(European_Union ~ s(gdp,bs='cr') + s(pop,bs='cr'),data=tr_data)
...
By looking at R squared and singular signifiance, the second one seems the best, let's go with it
```{r}
...
```{r}
consump.oil.petr.gam.1 <- gam(dif_oil_eu ~ s(dif_gdp_eu,bs='cr') + dif_pop_eu,data = vec_oil.diff) 
...
```
```{r}
consump.oil.petr.gam.2 <- gam(dif_oil_eu ~ dif_gdp_eu + s(dif_pop_eu,bs='cr'),data = vec_oil.diff) 
...
```{r}
consump.oil.petr.gam.3 <- gam(dif_oil_eu ~ dif_gdp_eu + dif_pop_eu,data = vec_oil.diff) 
...
consump.gas.gam.preds <- predict(consump.gas.gam, newdata=data.frame(vec_gas_pred.diff,inter=vec_gas_pred.diff$dif_gdp_eu*vec_gas_pred.diff$dif_pop_eu), se=T)
```
