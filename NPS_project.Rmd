...
REVIEW: Create dummy variables for countries
...
vec_train <- cbind(gdp.train[,1], pop.elec.train[,1])
for (j in 2:29){
    v <- cbind(gdp.train[,j], pop.elec.train[,j])
    vec_train <- rbind(vec_train,v)
}
vec_train <- data.frame(vec_train)
...
vec_test <- cbind(gdp.elec.pred[,1], pop.elec.pred[,1])
for (j in 2:11){
    v <- cbind(gdp.elec.pred[,j], pop.elec.pred[,j])
    vec_test <- rbind(vec_test,v)
}
vec_test <- data.frame(vec_test)
...
vec_oil.train <- cbind(vec_oil[1:len(vec_train)], vec_train)
vec_fos.train <- cbind(vec_fos[1:len(vec_train)], vec_train)
vec_gas.train <- cbind(vec_gas[1:len(vec_train)], vec_train)
```
...
Oil & Petr.
```{r}
consump.oil.petr.gam <- gam(value_oil ~ gdp + pop,
                           data = vec_oil.train) 
...
Fossils:
```{r}
consump.fossils.gam <- gam(value_ff ~ gdp + pop,
                           data = vec_ff.train) 
...
Natural gas:
```{r}
consump.gas.gam <- gam(value_ng ~ gdp + pop,
                           data = vec_ng.train) 
...
```{r}
data_ssp=read.xlsx("data/ssp_iiasa_YtY_gdp.xlsx",sheet=1)
```


```{r}
nrg_balances=read.xlsx('data/complete_energy_balances.xlsx',sheet=1)
View(nrg_balances)
```
...
```{r}
percentage_renew=read.xlsx('data/percentage_renewables.xlsx')
...
plot(ecdf(T_stat),xlim=c(-2,1),main="Wilks stat ecdf")
abline(v=T0,col=3,lwd=4)
...
```{r}
wind.mean = colMeans(use_wind_elec[4:40,]) #combustible fuels
solar.mean = colMeans(use_solar_elec[4:40,]) #renewables
years_04_19=as.numeric(colnames(use_wind_elec)) #years from 2004 to 2019

matplot(years_04_19,t(rbind(wind.mean,solar.mean)), type='l', col=c("green","red"), lty=1,xlab="Year",ylab="Gigawatt-hour",main="Mean electricity production")
legend("topleft",legend=c("Wind","Solar"),col=c(3,2),pch=1)
```

```{r}
n1 = n2=dim(use_wind_elec[4:40,])[1]
n  = n1 + n2
...
T2 = numeric(B)
set.seed(seed)
t_pooled = rbind(use_wind_elec[4:40,],use_solar_elec[4:40,])
...
loc_test=IWT2(as.matrix(use_wind_elec),as.matrix(use_solar_elec),B=B,alternative="greater")
plot(loc_test,main="Solar and Wind",col=c(2,3))
```
...
```
```{r}
dif_use_wind_elec=use_wind_elec[,-1]-use_wind_elec[,-length(use_wind_elec)]
```
...
list_vec=flatten(dif_use_wind_elec[4:40,])
...
matlines(year.grid ,c_preds_wind$lo ,lwd =1, col =" green",lty =3)
```
```{r}
pos=rep(c(TRUE,rep(FALSE,9)),15)
pos=c(pos,rep(FALSE,51))
predictions_wind=c_preds_wind$pred[pos]
res_belgium=as.numeric(dif_use_wind_elec[20,]-predictions_wind)
plot(res_belgium)
```
...
rownames(capacity_renew_fuels_nuclear)[which.max(capacity_renew_fuels_nuclear[4:38,30])+3] #first three rows are total european statistics
```
>>>>>>> 09ac14df0a5e659742715421bd7b886090413c18
