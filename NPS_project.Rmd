...
Use of electricity from renewables dataset
...
for (j in 1:16){
  use_principal[,j]=as.numeric(use_principal[,j])
}
...
```{r}
x=as.numeric(colnames(use_renew_elec))

plot(x,use_renew_elec[2,],main="Total electricity from renewables - Europe",xlab="Years",ylab="Gigawatt-hour")
```
```{r}
...
plot(x ,as.numeric(use_renew_elec[2,]) ,xlim=range(years.grid) ,cex =.5, col =" darkgrey ",main='Custom cut Fit',xlab="Years",ylab="Gigawatt-hour")
...
seed = 26
for (j in 1:30){
   capacity_comb_fuels[,j]=as.numeric(capacity_comb_fuels[,j])
}
...
years=as.numeric(colnames(capacity_comb_fuels))

matplot(years,t(rbind(cf.mean,ren.mean)), type='l', col=c("red","green"), lty=1,ylab="Megawatt",main="Mean production capacity")
...
```{r}
plot(years,as.numeric(capacity_comb_fuels[2,]),main="European Non Renewable Electricity Capacity",ylab="Megawatt")
```
...
```{r}
plot(years,as.numeric(capacity_renew_fuels_no_nuclear[2,]),main="European Renewable Electricity Capacity",ylab="Megawatt")
```
...
se.bands=cbind(preds$fit +2* preds$se.fit ,preds$fit -2* preds$se.fit)
plot(years ,as.numeric(capacity_comb_fuels[2,]) ,xlim=range(anni), ylim=range(as.numeric(capacity_renew_fuels_no_nuclear[2,])),cex =.5, col =" darkgrey ",main='Degree 3 poly combustible, degree 4 poly renewables',ylab="Megawatt")
...
plot(years ,as.numeric(capacity_comb_fuels[2,]) ,xlim=range(anni) ,cex =.5, col =" darkgrey ",
     main="Spline regression capacity - Combustibles",ylab="Megawatt")
lines(anni,preds$fit ,lwd =2, col =" red")
matlines(anni, se.bands ,lwd =1, col =" red",lty =3)
```
```{r}
...
plot(years ,as.numeric(capacity_renew_fuels_no_nuclear[2,]) ,xlim=range(anni) ,cex =.5, col =" darkgrey ",
     main="Spline regression capacity - Renewables",ylab="Megawatt")
lines(anni,preds$fit ,lwd =2, col =" green")
matlines(anni, se.bands ,lwd =1, col =" green",lty =3)
```
Natural splines
...
```
Again, we don't have statistical evidence to say that electricity capacities from renewables have different distribution in the four regions of Europe (we will redo test with relative capacities, should make more sense)
