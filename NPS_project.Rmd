...
We can clearly see that the electricity production coming from renewable sources is effectively growing and in particular wind and solar are developing at the same rate, while hydro is the most stable and developed electricity source.
...
```{r}
#linear pop and interaction
consump.oil.petr.gam.2 <- lm(dif_oil_eu ~ bs(dif_gdp_eu) + dif_pop_eu + dif_gdp_eu:dif_pop_eu,data = vec_oil.diff) 
...
Note that it is quite strange that we should not reject the null hypothesis of the fundamental test of significance, while we reject the null hypothesis of the singular tests. Therefore we have to check the significance of the whole model, still exploiting a permutational test.
```{r}
#global test of significance 
T0_glob <- summary(consump.oil.petr.gam)$f[1]
n=dim(vec_oil.diff)[1]
T_H0glob <- numeric(B)
set.seed(seed)
B=1000
for(perm in 1:B){
  ...
  Y.perm.glob <- vec_oil.diff$dif_oil_eu[permutazione]
  T_H0glob[perm] <- summary(lm(Y.perm.glob ~ dif_gdp_eu + dif_pop_eu ,data = vec_oil.diff))$f[1]   }
sum(T_H0glob>=T0_glob)/B

```

The model is significant, so we can confidently proceed.
...
## Test for COUNTRY outliers in diff dataset 
...
## Test for YEAR outliers with predictions
...
```{r}
plot(seq(1991,2030), nr.past.preds[,1], col=ifelse(fit_MCD.4$mcd.wt,"black","red"),pch=19,ylab="Oil&Petr.", xlab="Years (1991-2019)")
plot(seq(1991,2030), nr.past.preds[,2], col=ifelse(fit_MCD.4$mcd.wt,"black","red"),pch=19,ylab="Fossil Fuels", xlab="Years (1991-2019)")
plot(seq(1991,2030), nr.past.preds[,3], col=ifelse(fit_MCD.4$mcd.wt,"black","red"),pch=19,ylab="Natual Gas", xlab="Years (1991-2019)")
```
...
## Test for YEAR outliers without predictions
...
```{r}
plot(seq(1991,2019), nre.diff.sd.total[,1], col=ifelse(fit_MCD.7$mcd.wt,"black","red"),pch=19,ylab="Oil&Petr.", xlab="Years (1991-2019)")
plot(seq(1991,2019), nre.diff.sd.total[,2], col=ifelse(fit_MCD.7$mcd.wt,"black","red"),pch=19,ylab="Fossil Fuels", xlab="Years (1991-2019)")
plot(seq(1991,2019), nre.diff.sd.total[,3], col=ifelse(fit_MCD.7$mcd.wt,"black","red"),pch=19,ylab="Natural Gas", xlab="Years (1991-2019)")
...
### Redo DIFF_RENEWABLES ~ DIFF_NONRENEWABLES model
...
### Redo DIFF_GHG ~ DIFF_RENEWABLES model
...
c(Percentage_decrease.rob,Percentage_decrease)
...
points(seq(2020,2030), Percentage_emissions.rob, cex=1,pch=17,col="gold1")
lines(seq(1990,2030),rep(45,length(seq(1990,2030))),col="green",lty=2,lwd=2)

legend("topright",legend=c("Old predictions","New predictions"),col=c("red","gold1"),pch=16)
```
...
with(ghg, plot(seq(1990,2019) ,ValueGhg ,xlim=c(1990,2030),ylim=c(40,100),cex =.7,pch=19,xlab="Year",ylab="Percentage" ,main='Prediction - Ghg Emissions'))
points(seq(2020,2030),Percentage_emissions, cex=.7,col="red",pch=19)
points(seq(2020,2030),Percentage_emissions.lts,cex=.7,col="gold1",pch=19)
lines(seq(1990,2030),rep(45,length(seq(1990,2030))),lty=2,lwd=2,col="green")
legend("topright",legend=c("Old predictions","New predictions"),col=c("red","gold1"),pch=19)
```
...
Our work, that highlighted the non-feasibility of the Green Deal target of a 55% cut in Greenhouse gases emissions by 2030 respect to 1990, wants to be yet another raised alarm, to be added to the already long list of scientific researches on this subject.
