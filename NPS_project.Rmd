...
### Regression on types of RE for electricity production 
...
Since working with original data gave us residuals with high correlations, we decided to fit models using the year to year variations. We explot again GAM models to realize our regression task.
...
Smoothing for solar, wind and hydro to obtain semestral data (variations)
```{r}
...
Let's try with quadratic spline regression
```{r}
#combustible fuels
...
```{r}
library(lmPerm)
lmp=aovp(cap_19 ~ Region,perm="Prob",Cp=1e-6) 
summary(lmp)
...
```
From the third, let's try to quit the interaction (residuals are gaussian, but simple anova doesn't work)
...
The gam machinery suggest us that it is sufficient a simple linear model!
```{r}
...
A cut of 32.32% is expected with the actual rhythm, so more effort is needed in order to comply with the Green Deal objective!
...
### a.2. For Renewable energy consumption 
...
### a.3. For extra-covariates GDP & Population
...
```{r}
plot(seq(1991,2030), nr.past.preds[,1], col=ifelse(fit_MCD.4$mcd.wt,"black","red"),pch=19)
plot(seq(1991,2030), nr.past.preds[,2], col=ifelse(fit_MCD.4$mcd.wt,"black","red"),pch=19)
plot(seq(1991,2030), nr.past.preds[,3], col=ifelse(fit_MCD.4$mcd.wt,"black","red"),pch=19)
```
...
### b.1.2. Renewable Energy consumption (training + prediction)

REVIEW: Using GAM fitted values and not real train values!

WARNING: "...However, as an estimator for univariate location and scale the MCD is
outperformed by other methods (in terms of robustness and efficiency).
Therefore the MCD is mainly useful for higher-dimensional data." ()[https://cseweb.ucsd.edu/~slovett/workshops/robust-statistics-2019/slides/donoho-multivariate.pdf]

```{r}
r.past.preds <- data.frame(
    c(consump.renew.gam$fitted.values, consump.renew.gam.preds$fit))

rownames(r.past.preds) <- c(seq(1991,2019,by=1/3), seq(2020,2030,by=1/3))
colnames(r.past.preds) <- c("renewables")
```

```{r}
## N -> each year 
## p -> 1 (Renewable energy consumption) 
alpha <- .75
fit_MCD.5 <- covMcd(x = r.past.preds, alpha = alpha, nsamp = "best")
plot(fit_MCD.5)

# Which are the cases considered outliers from distance plots?
rownames(r.past.preds)[!(fit_MCD.5$mcd.wt)]
```



### b.1.3. GHG (training + prediction)
...
```{r}
ghg.past.preds <- data.frame(
    c(ghg.l$fitted.values, ghg.l.preds$fit))

rownames(ghg.past.preds) <- c(seq(1991,2019), seq(2020,2030))
colnames(ghg.past.preds) <- c("ghg")
```
...
## p -> 1 (Renewable energy consumption)
alpha <- .75
fit_MCD.6 <- covMcd(x = ghg.past.preds, alpha = alpha, nsamp = "best")
plot(fit_MCD.6)

# Which are the cases considered outliers from distance plots?
rownames(ghg.past.preds)[!(fit_MCD.6$mcd.wt)]
```

## b.2. Test for YEAR outliers without predictions

Let's try to find outlying years from our NRE consumption data which is .
...
```{r}
plot(seq(1991,2019), nre.diff.sd.total[,1], col=ifelse(fit_MCD.7$mcd.wt,"black","red"),pch=19)
plot(seq(1991,2019), nre.diff.sd.total[,2], col=ifelse(fit_MCD.7$mcd.wt,"black","red"),pch=19)
plot(seq(1991,2019), nre.diff.sd.total[,3], col=ifelse(fit_MCD.7$mcd.wt,"black","red"),pch=19)
...
Both have been done already. In one we obtain 7 outliers, while in this last methodology we obtain 2. Inspecting the indiviudal graphs and the distance-distance plots, we can assume that it is better to choose the outliers detected in the current section since the 2011 and 2014 yearly NRE consumptions are much clearer outliers than extra ones detected in the b.1.1. section.

We can infere that the extra outliers from section b.1.1. are selected as such because of the extra-centrality that the predictions of NRE consumptions add to the dataset. It is clear that a swamping effect is taking place because some variabilities are amplified and falsely tagged as outlying. 

### b.2.2. Redo DIFF_RE' ~ DIFF_NRE' model
...
```{r}
consump.renew.gam.rob <- gam(value_re ~ s(value_oil,bs='cr') + s(value_ff,bs='cr') + s(value_ng,bs='cr') + s(I(value_oil*value_ff),bs='cr') + s(I(value_ff*value_ng),bs='cr') + s(I(value_oil*value_ng),bs='cr'), data=vec_re.rob.s)
...
```{r}
consump.renew.gam.rob <- gam(value_re ~ s(value_oil,bs='cr') + s(value_ff,bs='cr') + s(value_ng,bs='cr') + s(I(value_oil*value_ff),bs='cr') + s(I(value_oil*value_ng),bs='cr'), data=vec_re.rob.s)
...
```{r}
consump.renew.gam.rob <- gam(value_re ~ s(value_oil,bs='cr') + s(value_ff,bs='cr') + s(value_ng,bs='cr') + s(I(value_oil*value_ng),bs='cr'), data=vec_re.rob.s)
...
```{r}
consump.renew.gam.rob <- gam(value_re ~ s(value_oil,bs='cr') + s(value_ff,bs='cr') + s(value_ng,bs='cr') + s(I(value_oil*value_ff),bs='cr'), data=vec_re.rob.s)
...
```{r}
consump.renew.gam.rob <- gam(value_re ~ s(value_oil,bs='cr') + s(value_ff,bs='cr') + s(value_ng,bs='cr'), data=vec_re.rob.s)
...
```{r}
consump.renew.gam.rob <- gam(value_re ~ s(value_oil,bs='cr') + s(value_ng,bs='cr'), data=vec_re.rob.s)
...
shapiro.test(consump.renew.gam.rob$residuals)
```
This last model still has an okey p-value but R-squared decreases too much. Best model seems The one consisting of the three variables and no interactions. 
...
### b.2.3. Redo DIFF_GHG' ~ DIFF_RE' model
...
Residuals are not normal. A GAM model does not seem to be okey.
Let's try as before to use a linear model. A linear model will result in exactly the same output since we are using a single regressor:
...
### b.2.4. Compare with original models
...
with(vec_re, plot(seq(1991,2019) ,value_re ,xlim=c(1991,2030),cex=.5,xlab="Year",ylab="Terajoule" , col =" darkgrey ",main='Prediction - Renewable Energy Consumption'))
points(seq(2020,2030,1/3), consump.renew.gam.preds$fit ,cex=1, pch=24, col ="coral")
lines(seq(1991,2019,1/3),consump.renew.gam$fitted.values,col="lightblue",lwd=1)

# Plot new "robust" predictions
points(1990+outl.nre.train, vec_re$value_re[outl.nre.train], col =" black ") # Outliers
points(seq(2020,2030,1/3), consump.renew.gam.preds.rob$fit ,cex=1, pch=25, col ="brown")
lines(seq.outl.s,consump.renew.gam.rob$fitted.values,col="blue",lwd=1)
...
points(seq(2020,2030),ghg.l.preds$fit ,cex=0.5, pch=24, col ="coral")
lines(seq(1991,2019),ghg.l$fitted.values,lwd=1,col="lightblue")
lines(seq(1991,2030,1/3),rep(0,length(seq(1991,2030,1/3))),col="darkgrey",lty=2)

# Plot new "robust" predictions
points(1990+outl.nre.train,vec_ghg.diff.2$value_ghg[outl.nre.train],xlim=c(1990,2030),cex =.5,col =" darkgrey ")
points(seq(2020,2030),ghg.rob.preds$fit ,cex=0.5, pch=25, col ="brown")
lines(seq.outl,ghg.rob$fitted.values,lwd=1,col="blue")
lines(seq(1991,2030,1/3),rep(0,length(seq(1991,2030,1/3))),col="darkgrey",lty=2)
...
with(vec_ghg.diff,plot(value_re,value_ghg,main="Diff ghg vs diff renewables"))
# Previous regression
lines(newdata$value_re,ghg.reg$fit,col="darkgreen",lwd=2)
matlines(newdata$value_re,se.bands,col="darkgreen",lwd=1,lty=2)
# New regression
lines(newdata$value_re, ghg.reg.rob$fit, col="green", lwd=2)
matlines(newdata$value_re, se.bands.rob, col="green", lwd=1, lty=2)
...
Plot predictions
```{r}
with(ghg, plot(seq(1990,2019) ,ValueGhg ,xlim=c(1990,2030),ylim=c(40,100),cex =.5,xlab="Year",ylab="Percentage" , col =" darkgrey ",main='Prediction - Ghg Emissions'))
points(seq(2020,2030),Percentage_emissions,cex=.5,pch=24,col="darkgreen")
points(seq(2020,2030), Percentage_emissions.rob, cex=.5,pch=25,col="green")
lines(seq(1990,2030),rep(45,length(seq(1990,2030))),col="darkgrey",lty=2)
```
...
### c.1. Least Median Squares Regression
...
### c.2. Least Trimmed Squares Regression
