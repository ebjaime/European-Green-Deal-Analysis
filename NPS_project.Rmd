...
Let's find the adequate model to use for conformal prediction
```{r}
...
Pacf(model_gam$residuals)
```
Some high residuals but we stick with this model
```{r}
...
```{r}
Co2=read.xlsx("data/ghg_emissions.xlsx",sheet=3,colNames=F)
Co2=data.frame(t(Co2))
rownames(Co2)=years_90_19
colnames(Co2)="ValueCo2"
head(Co2)
...
```{r}
plot(years_90_19,Co2$ValueCo2)
```


Core ok until here!!!
```{r}
Co2.diff=data.frame(Co2[-1,]-Co2[-30,])
colnames(Co2.diff)="diff_co2"
```

```{r}
plot(seq(1991,2019),Co2.diff$diff_co2)
```


```{r}
#using differences for covariates
vec_c02= cbind(Co2.diff$diff_co2,vec_re$value_re,vec_oil$dif_gdp_eu,vec_oil$dif_pop_eu)
vec_c02 <- data.frame(vec_c02)
vec_c02_pred <- data.frame(t(renew.predictions),vec_oil_pred$dif_gdp_eu,vec_oil_pred$dif_pop_eu)
...
```{r}
rownames(vec_c02)=years_90_19[2:30]
vec_c02.temp=t(vec_c02)
vec_c02.s=smoothing_procedure(vec_c02.temp,1991,2019,interval=3)
rownames(vec_c02.s)=rownames(vec_c02.temp)
vec_c02.s=data.frame(t(vec_c02.s))
View(vec_c02.s)
```

```{r}
vec_c02_pred.temp=t(vec_c02_pred)
vec_c02_pred.s=smoothing_procedure(vec_c02_pred.temp,2020,2030,interval=3)
rownames(vec_c02_pred.s)=rownames(vec_c02_pred.temp)
vec_c02_pred.s=data.frame(t(vec_c02_pred.s))
```


```{r}
c02.gam <- gam(value_c02 ~ s(value_re,bs='cr') + s(value_gdp,bs='cr') + s(value_pop,bs='cr') + s(I(value_re*value_gdp),bs='cr') + s(I(value_gdp*value_pop),bs='cr') + s(I(value_re*value_pop),bs='cr') + s(I(value_re*value_gdp*value_pop),bs='cr'), data=vec_c02.s)
...
Let's quit gdp and gdp-pop interaction

```{r}
c02.gam <- gam(value_c02 ~ s(value_re,bs='cr')  + s(value_pop,bs='cr') + s(I(value_re*value_gdp),bs='cr')  + s(I(value_re*value_pop),bs='cr') + s(I(value_re*value_gdp*value_pop),bs='cr'), data=vec_c02.s)
...
```
```{r}
c02.gam <- gam(value_c02 ~ s(value_re,bs='cr'), data=vec_c02)
...
with(vec_c02, plot(seq(1991,2019) ,value_c02 ,xlim=c(1990,2030),cex =.5,xlab="Year",ylab="Terajoule" , col =" darkgrey ",main='GAM Prediction - Co2 Emissions'))
points(seq(2020,2030),c02.gam.preds$fit ,cex=0.5, col ="red")
lines(seq(1991,2019),c02.gam$fitted.values,lwd=1,col="blue")
```



```{r}
c02.gam.preds <- predict(c02.gam, newdata=vec_c02_pred.s, se=T)

with(vec_c02.s, plot(seq(1991,2019,1/3) ,value_c02 ,xlim=c(1990,2030),ylim=c(-1000,20),cex =.5,xlab="Year",ylab="Terajoule" , col =" darkgrey ",main='GAM Prediction - Co2 Emissions'))
